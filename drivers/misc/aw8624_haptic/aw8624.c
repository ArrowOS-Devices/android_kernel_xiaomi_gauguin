...
static int aw8624_rtp_trim_lra_calibration(struct aw8624 *aw8624)
{
	...
	printk(KERN_DEBUG, "microsecond:%ld  theory_time = %d\n", aw8624->microsecond,
	       theory_time);
...
static int aw8624_haptic_f0_calibration(struct aw8624 *aw8624)
{
	...
	if (aw8624_haptic_get_f0(aw8624)) {
		...
		if (aw8624->chipid_flag == 1) {
			printk(KERN_DEBUG, "%s  %d aw8624->chipid_flag = 1 \n", __func__,
			       __LINE__);
			if (f0_cali_step > 31)
			   ...
			   struct device_node *np)
{
	...
	//if (val != 0)
	//      printk(KERN_DEBUG, "%s vib_trig_config not found\n", __func__);
	...
	for_each_available_child_of_node(np, child_node) {
		...
		rc = of_property_read_u32(child_node, "qcom,effect-id",
					  &effect->id);
		if (rc != 0) {
			printk(KERN_DEBUG, "%s: Read qcom,effect-id failed\n", __func__);
		}
		printk(KERN_DEBUG, "%s: effect_id: %d\n", __func__, effect->id);

		effect->vmax_mv = config->vmax_mv;
		rc = of_property_read_u32(child_node, "qcom,wf-vmax-mv", &tmp);
		if (rc != 0)
			printk(KERN_DEBUG, "%s:  Read qcom,wf-vmax-mv failed !\n", __func__);
		else
			effect->vmax_mv = tmp;

		printk(KERN_DEBUG, "%s: ---%d effect->vmax_mv =%d \n", __func__, __LINE__,
		       effect->vmax_mv);
		rc = of_property_count_elems_of_size(child_node,
						     "qcom,wf-pattern",
						     sizeof(u8));
		if (rc < 0) {
			printk(KERN_DEBUG, "%s: Count qcom,wf-pattern property failed !\n",
			       __func__);
		} else if (rc == 0) {
			printk(KERN_DEBUG, "%s: qcom,wf-pattern has no data\n", __func__);
		}
		...
		rc = of_property_read_u8_array(child_node, "qcom,wf-pattern",
					       ...
					       effect->pattern_length);
		if (rc < 0) {
			printk(KERN_DEBUG, "%s: Read qcom,wf-pattern property failed !\n",
			       __func__);
		}
		printk
		    (KERN_DEBUG, "%s: %d  effect->pattern_length=%d  effect->pattern=%d \n",
		...
		rc = of_property_read_u32(child_node, "qcom,wf-play-rate-us",
					  &tmp);
		if (rc < 0)
			printk(KERN_DEBUG, "%s: Read qcom,wf-play-rate-us failed !\n",
			       __func__);
		else
			effect->play_rate_us = tmp;
		printk(KERN_DEBUG, "%s: ---%d effect->play_rate_us=%d \n", __func__,
		       __LINE__, effect->play_rate_us);

		rc = of_property_read_u32(child_node, "qcom,wf-repeat-count",
					  &tmp);
		if (rc < 0) {
			printk(KERN_DEBUG, "%s: Read  qcom,wf-repeat-count failed !\n",
			       __func__);
		} else {
		...
		rc = of_property_read_u32(child_node, "qcom,wf-s-repeat-count",
					  &tmp);
		if (rc < 0) {
			printk(KERN_DEBUG, "%s: Read  qcom,wf-s-repeat-count failed !\n",
			       __func__);
		} else {
		...
		if (tmp > HAP_BRAKE_PATTERN_MAX) {
			printk
			    (KERN_DEBUG, "%s: wf-brake-pattern shouldn't be more than %d bytes\n",
		...
		rc = of_property_read_u8_array(child_node,
					       ...
					       effect->brake, tmp);
		if (rc < 0) {
			printk(KERN_DEBUG, "%s: Failed to get wf-brake-pattern !\n",
			       __func__);
		}
	...
	for (j = 0; j < i; j++) {
		...
		printk(KERN_DEBUG, "%s:       vmax: %d mv\n", __func__,
		       aw8624->predefined[j].vmax_mv);
		printk(KERN_DEBUG, "%s:        play_rate: %d us\n", __func__,
		       aw8624->predefined[j].play_rate_us);
		for (m = 0; m < aw8624->predefined[j].pattern_length; m++)
			printk(KERN_DEBUG, "%s:     pattern[%d]: 0x%x\n", __func__, m,
			       aw8624->predefined[j].pattern[m]);
		for (m = 0; m < aw8624->predefined[j].brake_pattern_length; m++)
			printk(KERN_DEBUG, "%s:     brake_pattern[%d]: 0x%x\n", __func__, m,
			       aw8624->predefined[j].brake[m]);
		printk(KERN_DEBUG, "%s:         brake_en: %d\n", __func__,
		       aw8624->predefined[j].brake_en);
		printk(KERN_DEBUG, "%s:        wf_repeat_n: %d\n", __func__,
		       aw8624->predefined[j].wf_repeat_n);
		printk(KERN_DEBUG, "%s:         wf_s_repeat_n: %d\n", __func__,
		       aw8624->predefined[j].wf_s_repeat_n);
		printk(KERN_DEBUG, "%s:         lra_auto_res_disable: %d\n", __func__,
		       aw8624->predefined[j].lra_auto_res_disable);
	}
	printk(KERN_DEBUG, "%s:       aw8624->effects_count: %d\n", __func__,
	       aw8624->effects_count);
	printk(KERN_DEBUG, "%s:       aw8624->effect_id_boundary: %d\n", __func__,
	       aw8624->info.effect_id_boundary);
	printk(KERN_DEBUG, "%s:       aw8624->effect_max: %d\n", __func__,
	       aw8624->info.effect_max);
	printk(KERN_DEBUG, "%s:       aw8624->info.cont_drv_lvl: %d\n", __func__,
	       aw8624->info.cont_drv_lvl);
	printk(KERN_DEBUG, "%s:       aw8624->info.cont_drv_lvl_ov: %d\n", __func__,
	       aw8624->info.cont_drv_lvl_ov);
	printk(KERN_DEBUG, "%s:       aw8624->info.gain_flag: %d\n", __func__,
	       aw8624->info.gain_flag);
	for (i = 0; i < 3; i++)
		for (j = 0; j < 5; j++)
			printk
			    (KERN_DEBUG, "%s:       aw8624->info.trig_config[%d][%d]: %d\n", __func__,
			     i, j, aw8624->info.trig_config[i][j]);
	for (i = 0; i < 175; i++)
		printk(KERN_DEBUG, "%s:       aw8624->info.rtp_time[%d]: %d\n", __func__,
		       i, aw8624->info.rtp_time[i]);
	printk(KERN_DEBUG, "%s:       aw8624->info.parameter1: 0x%x\n", __func__,
	       aw8624->info.parameter1);
	return 0;
}
					...
					struct ff_effect *old)
{
	...
	if (hrtimer_active(&aw8624->timer)) {
		...
		printk(KERN_DEBUG, "waiting for playing clear sequence: %lld us\n",
		       time_us);
	...
	if (aw8624->effect_type == FF_CONSTANT) {
		...
		//printk(KERN_DEBUG, "%s  %d  aw8624->play.vmax_mv = 0x%x\n", __func__, __LINE__, aw8624->play.vmax_mv);
...
static int aw8624_hw_reset(struct aw8624 *aw8624)
{
	...
	if (!aw8624->enable_pin_control) {
		if (aw8624 && gpio_is_valid(aw8624->reset_gpio)) {
			...
			printk(KERN_DEBUG, "%s pull down1\n", __func__);
			msleep(5);
			gpio_set_value_cansleep(aw8624->reset_gpio, 1);
			printk(KERN_DEBUG, "%s pull up1\n", __func__);
			msleep(5);
		} else {
...
aw8624_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (IS_ERR(aw8624->aw8624_pinctrl)) {
		if (PTR_ERR(aw8624->aw8624_pinctrl) == -EPROBE_DEFER) {
			printk(KERN_DEBUG, "pinctrl not ready\n");
			rc = -EPROBE_DEFER;
			return rc;
		}
		printk(KERN_DEBUG, "Target does not use pinctrl\n");
	...
	for (i = 0; i < ARRAY_SIZE(aw8624->pinctrl_state); i++) {
		...
		if (IS_ERR(state)) {
			printk(KERN_DEBUG, "cannot find '%s'\n", n);
			rc = -EINVAL;
			//goto exit;
		}
